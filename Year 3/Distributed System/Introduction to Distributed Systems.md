Distributed System is learning about mechanism that allow computers to with each other interact.
Multiple processor/machine that work together, handling work [[#Parallel System|parallelly]] to finish it.

### Distributed Architecture
A computer is an encapsulated system of Processors, the Processor communicate with I/O Device, the Memory, and Storage.
With better processor, it allow for more transistor at a time, more transistor = more computation, but also more heat -> We have to create better cooling system -> Up to a point that there's so much transistor that they started to fry each other (Ref: The drop of Performance and Frequency after 2005).
To create an application, you need to consider every "computer" that exist, since the failure of one random computer can lead to the failure of your own computer

A Distributed System has 3 requirement/abstraction that is to be distributed:
- Storage: File can be stored in multiple location, and each part of the file can be indexed to re-create a file
- Communication: The ability to chat from anywhere in the world and send messages with little to no delay by communication of server across the world
- Computation: A big task/computation can be divided/splitted to smaller tasks and distributed to multiple worker
